		...
		<view class="product-con skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
				...
				<scroll-view :scroll-top="scrollTop" scroll-y='true' scroll-with-animation="true" :style='"height:"+height+"px;"'
				 @scroll="scroll">
					<view id="past0">
						...
						<view class='wrapper'>
							...
							<view class='integral_count skeleton-rect'>
								...
								<text v-if="storeInfo.delivery_free == 1" class='integral'>{{$t(`page.goodsDetail.shipping`)}}</text>
							</view>
							<view class='introduce skeleton-rect'>
								...
								<text v-else-if="storeInfo.merchant.is_trader" class="font-bg-red bt-color">{{$t(`page.store.selfSupport`)}}</text>
							...
							<view class='label acea-row row-between-wrapper' :style="{'padding-bottom':coupon.list.length?0:10+'px'}">
								<view class="skeleton-rect">{{$t(`page.goodsDetail.MarketPrice`)}}:${{storeInfo.ot_price ? storeInfo.ot_price : ''}}</view>
								<view class="skeleton-rect">{{$t(`page.goodsDetail.inventory`)}}:{{storeInfo.stock ? storeInfo.stock : 0}}</view>
								<view class="skeleton-rect">{{$t(`page.goodsDetail.sales`)}}:{{storeInfo.sales ? storeInfo.sales : 0}}</view>
							</view>
							...
							<view class="store-info">
								...
								<view class="info">
									<view class="name line1">
										...
										<text v-else-if="storeInfo.merchant.is_trader" class="font-bg-red ml8 bt-color">{{$t(`page.store.selfSupport`)}}</text>
									</view>
									...
									<view v-else class="txt">0{{$t(`page.store.people`)}}</view>
								</view>
								<navigator v-if="hide_mer_status != 1" :url="'/pages/store/index?id='+storeInfo.merchant.mer_id" class="link"
								 hover-class="none">{{$t(`page.goodsDetail.click`)}}</navigator>
							</view>
					...
					<view class='product-intro' id="past3">
						<view class='title'>{{$t(`page.goodsDetail.details`)}}</view>
			...
			<view class='footer acea-row row-between-wrapper'>
				<view v-if="storeInfo.merchant && storeInfo.merchant.services_type == 1" class="item skeleton-rect" @click="call">
					...
					<view>{{showSkeleton ? 'xxxxxx' : $t(`page.goodsDetail.contact`)}}</view>
				</view>
				...
				<view @click="setCollect" class='item skeleton-rect'>
					...
					<view>{{showSkeleton ? 'xxxxxx' : $t(`page.goodsDetail.like`)}}</view>
				</view>
				<navigator open-type='switchTab' class="animated item skeleton-rect" :class="animated==true?'bounceIn':''" url='/pages/order_addcart/order_addcart'
				 hover-class="none">
					...
					<view>{{showSkeleton ? 'xxxxxx' : $t(`page.goodsDetail.cart`)}}</view>
				</navigator>
				<view v-if="attr.productSelect.stock != 0" class='bnt acea-row skeleton-rect'>
					<form @submit="joinCart" report-submit='true'>
						<button v-if="storeInfo.type != 1" class='joinCart bnts' form-type="submit">{{$t(`page.goodsDetail.addCart`)}}</button>
					</form>
					<form @submit="goBuy" report-submit='true' v-if="attr.productSelect">
						<button class='buy bnts' :class="storeInfo.type == 1 ? 'virtual_buy' : ''" form-type="submit" :disabled="attr.productSelect.stock == 0">{{$t(`page.goodsDetail.buyNow`)}}</button>
					</form>
				</view>
