		...
		<view class="new-users skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
			<view class="head">
				<view class="user-card">
					...
					<view class="user-info">
						<image class="avatar skeleton-radius" :src="userInfo.avatar ? userInfo.avatar : '/static/images/f.png'"></image>
						<view class="info">
							...
							<view class="num" v-if="userInfo.invite_code" @click="goEdit">
								<view class="num-txt">ID：{{userInfo.invite_code}}</view>
							</view>
						</view>
					</view>
					<view class="num-wrapper skeleton-rect">
						<view class="num-item" @click="goMenuPage('/pages/users/user_money/index')">
							...
							<view class="txt">{{$t(`page.user.money`)}}</view>
						</view>
						...
						<view v-if="hide_mer_status == 0" class="num-item" @click="goMenuPage('/pages/users/user_store_attention/index')">
							...
							<view class="txt">{{$t('page.user.favstore')}}</view>
						</view>
					</view>
					<view class="right-btn">
						<view class="iconfont icon-shezhi" v-if="isLogin" @click="goEdit()"></view>
			...
			<view class="wrapper">
				...
				<view class="user-menus" style="margin-top: 20rpx;">
					<view class="title skeleton-rect">{{$t('page.user.services')}}</view>
					<view class="menu-box">
						<block v-for="(item,index) in personalMenu" :key="index">
							<view v-if="item.isShow && item.name" class="item">
								<view @click="authTo(item.url)" class="item-count">
									...
									<text>{{$t('page.user.'+item.name)}}</text>
								</view>
							</view>
						</block>
					</view>
				...
				<view class="menus-list-item" v-if="!userInfo.topService && userInfo.service" @click="toService(1)">
					<view class="item-text">
						<view class="title">
							<text class="merchant">{{$t('page.user.merchant')}}</text>{{$t('page.user.management')}}
						</view>
						<view class="info">{{$t('page.user.managementCenter')}}</view>
					</view>
...
<script>
	...
	export default {
		...
		computed: {
			orderMenu(){
				return this.$t('page.user.orderMenu')
			},
			...configMap({
		...
		methods: {
			...
			filterMenus: function(item){
				...
				else if(item.key=='/pages/store/settled/index') item.isShow = that.mer_intention_open == 1 && that.hide_mer_status != 1
			...
			orderNum() {
				orderData().then(({
					...
					this.orderMenu.forEach((item, index) => {
						switch (item.title) {
							case this.$t(`page.user.orderStatus[0].name`):
								item.num = data.noPay
								break
							case this.$t(`page.user.orderStatus[1].name`):
								item.num = data.noPostage
								break
							case this.$t(`page.user.orderStatus[2].name`):
								item.num = data.noDeliver
								break
							case this.$t(`page.user.orderStatus[3].name`):
								item.num = data.noComment
								break
							case this.$t(`page.user.orderStatus[4].name`):
