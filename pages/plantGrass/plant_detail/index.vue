						...
						<view v-if="followDetail.author" class="title">
							...
							<view class="author">
								<view class="manageCount" v-if="followDetail.is_author">
									<view class="follow_btn" @click.stop="showManage=!showManage">{{$t('page.goodsAddcart.management')}}</view>
									<view class="manage" v-show="showManage">
										<navigator hover-class="none" :url="'/pages/plantGrass/plant_release/index?id='+id" class="items">
											...
											<text>{{$t(`message.tips.edit`)}}</text>
										</navigator>
										<view class="items" @click.stop="deleteTopic">
											<text class="iconfont icon-shanchu3"></text>
											<text>{{$t(`page.users.userAddressList.del`)}}</text>
										</view>
									</view>
								</view>
								<view v-else @click.stop="followAuthor">
									<view v-if="!followDetail.is_fans" class="follow_btn focus"><text class="iconfont icon-jiahao2"></text>{{$t(`page.store.follow`)}}</view>
									<view v-else class="follow_btn focused">{{$t(`page.store.followed`)}}</view>
								</view>
								<!-- #ifdef MP -->
		...
		methods: {
			...
			getReplyList(){
				...
				replyLstApi(that.id,that.where).then(res => {
					...
					that.loading = false;
					that.loadTitle = that.loaded ? this.$t(`page.goodsList.no`) :  this.$t(`page.goodsList.more`);
