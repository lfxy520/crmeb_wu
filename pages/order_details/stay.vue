		...
		<view class='order-details'>
			...
			<view v-if="orderInfo.orderList && orderInfo.orderList[0].activity_type != 2">
				<view class='header acea-row row-middle'>
					...
					<view class='data'>
						<view class='state'>{{$t(`message.orderConfirm.payLoading`)}}</view>
					</view>
				</view>
			...
			<view class='wrapper'>
				<view class='item acea-row row-between'>
					...
					<view class='conter acea-row row-middle row-right'>{{orderInfo.group_order_sn}}
						...
						<!-- #endif -->
						<!-- #ifdef H5 -->
						<text class='copy copy-data' :data-clipboard-text="orderInfo.group_order_sn">{{$t(`page.orderDetails.copy`)}}</text>
						<!-- #endif -->
					</view>
				</view>
				<view class='item acea-row row-between'>
				...
				<view class='item acea-row row-between'>
					...
					<view class='conter'>{{$t(`page.orderDetails.payFalse`)}}</view>
				</view>
				<view class='item acea-row row-between'>
			...
			<view class='wrapper'>
				...
				<!-- <view class='actualPay acea-row row-right'>{{$t(`page.orderDetails.actualPayment`)}}：<text class='money font-color'>{{orderInfo.pay_price}}</text></view> -->
			</view>
			<view style='height:120rpx;'></view>
			<view class='footer acea-row row-right row-middle'>
				<view class="bnt cancel" @click.stop="cancelOrder">{{$t(`page.orderDetails.cancelOrder`)}}</view>
				<view class='bnt bgColor' @tap='pay_open(orderInfo.order_id)'>{{$t(`page.orderDetails.nowPay`)}}</view>
			</view>
		</view>
		...
		methods: {
			...
			cancelOrder() {
				...
				uni.showModal({
					...
					content: this.$t(`page.orderDetails.confirmCancel`),
