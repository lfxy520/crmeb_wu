			  	...
			  	<block>
			  		<swiper-item :class="{ active: 0 == swiperCur }">
			  			<view class="slide-navigator">
			  				<navigator class="item" :url="`/pages/product/goods/goods?mer_id=0`" hover-class='none'>
			  				  <image mode='widthFix' class="image" src="../static/images/product_add.png"></image>
			  				  <text class="text">{{$t('page.product.pick')}}</text>
			  				</navigator>
			  				<view class="item" hover-class='none' @click="jumpAddGoods">
			  					<image mode='widthFix' class="image" src="../static/images/product_add.png"></image>
			  					<text class="text">{{$t('page.product.add')}}</text>
			  				</view>
			  				<navigator class="item" :url="`/pages/product/goodsOnSale/index?mer_id=${mer_id}&type=1`" hover-class='none'>
			  				  ...
			  				  <text class="text">{{$t('page.product.onsale')}}</text>
			  				</navigator>
			  				<navigator class="item" :url="`/pages/product/goodsOnSale/index?mer_id=${mer_id}&type=3`" hover-class='none'>
			  				  ...
			  				  <text class="text">{{$t('page.product.offsale')}}</text>
			  				</navigator>
			  				<navigator class="item" :url="`/pages/product/storeClassification/index?mer_id=${mer_id}`" hover-class='none'>
			  				  ...
			  				  <text class="text">{{$t('page.product.fenlei')}}</text>
			  				</navigator>		
			  			</view>
					</swiper-item>
			  	</block>
          ...
          <block v-for="(item,index) in productList" :key="index"> 
              <view class='item'>
				...
				<view class='text'>
				  ...
				  <view class="sales">
				    ...
				    <text class="num">{{$t('page.goodsDetail.sales')}}: {{item.sales}}</text>
				  </view>
            ...
            <view class="operation acea-row row-between-wrapper">
              ...
              <view class="acea-row row-middle">
                <view v-if="item.is_show == 1 && item.status == 1" class="bnt" @tap.stop="handleShelves(item,0)">{{$t('page.product.takeon')}}</view>
                <view v-if="item.is_show == 0 && item.status == 1" class="bnt" @tap.stop="handleShelves(item,1)">{{$t('page.product.takeoff')}}</view>
				<view @click="editGoods(item)" class="bnt">{{$t(`message.tips.edit`)}}</view>
              </view>
            </view>
          </block>
        </view>
...
export default {
  ...
  methods: {
    ...
    handleShelves(item, status) {
      ...
      if(status == 0) {
        uni.showModal({
          content: this.$t('page.product.tip10'),
          ...
          success: function(res) {
            if(res.confirm) {
              ...
              uni.showToast({
                title: this.$t('page.product.tip11'),
                icon: 'none'
              })
            } else if(res.cancel) {
              console.log('用户点击取消');
