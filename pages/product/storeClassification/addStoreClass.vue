	...
	<view class="container">
		<view class="input_content">
			<view class="input_content_textarea">
				<textarea v-model="formData.cate_name" :placeholder="$t('page.product.Category')" placeholder-class="placeholderStyle" maxlength="12"/>
		...
		<view class="list_content">
			<view class="list_item">
				<text class="list_item_title">{{$t('page.product.sjfenlei')}}</text>
				<picker @change="bindPickerChange" :range="classifyArr" range-key="cate_name">									
					<input placeholder="" type="text" disabled readonly v-model="parentClassify">
					<view class='iconfont icon-jiantou'></view>
				</picker>						
			</view>
			<view class="list_item">
				<view class="list_item_title">{{$t('page.product.show')}}</view>
		...
		getCateList(){
			storeClassifyLst(this.mer_id)
				.then(res => {
					...
					this.classifyArr.unshift({cate_name: this.$t('page.product.djfenlei'), store_category_id: 0, children: []})
