<template>
	<view class="area_container">
		<view class="area_container_title">
			<navigator :url="`/pages/product/storeClassification/index?mer_id=${mer_id}`" hover-class="none" class="manage_btn">{{$t('page.goodsAddcart.management')}}</navigator>		
			<view class="area_container_title_name">{{$t('page.product.selectit')}}{{$t('page.product.dpfenlei')}}</view>
		...
		<view class="area_container_content">
			...
			<view class="selectList_tap">
				...
				<view class="selectList_tap_item" @click="selectTapLastItem(-1)" v-if="isShowLastItem" :class="{ selectTap: selectTap == -1 }">{{$t('page.product.selectit')}}</view>
			</view>
			...
			<view class="handle"><view class="handle_button" @click="handleGetSelectArea">{{$t('page.goodsDetail.confirm')}}</view></view>
		</view>
	</view>
</template>
