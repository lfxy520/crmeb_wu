			...
			<view class="search">
				<view class="title_search">
					...
					<form @submit="search" report-submit="true">
						<input type="text" :name="where.keyword" v-model="where.keyword" :placeholder="$t(`page.goodsDetail.search`)" confirm-type='search'/>
					</form>		
				</view>
			</view>
			<view class="longTab">
					...
					<view class="shop_list_item">
						<view class="shop_list_item_shop">
							<view class="shop_list_item_shop_image">
								...
								<text v-if="item.spec_type == 1" class="spec">{{$t('page.product.dgg')}}</text>
							</view>
							<view class="shop_list_item_shop_con">
								...
								<view class="shop_list_item_shop_con_message">
									<span>{{$t('page.goodsDetail.inventory')}}：{{ item.stock }}</span>
									<span>{{$t('page.goodsDetail.sales')}}：{{ item.sales }}</span>
								</view>
						...
						<view class="shop_list_item_handle">
							<view v-if="item.is_show == 1 && item.status == 1 && type != 5" class="shop_button" @click="upperShelfAndLowerShelf(item, index)">{{$t('page.product.takeoff')}}</view>
							<view v-if="item.is_show == 0 && item.status == 1 && type != 5" class="shop_button" @click="upperShelfAndLowerShelf(item, index)">{{$t('page.product.takeon')}}</view>
							<view class="shop_button" v-if="type != 1 && type != 3" @click="handleDelete(item, index)">{{$t('page.product.delete')}}</view>
							<view class="shop_button" v-if="type == 5" @click="reduction(item, index)">{{$t('page.product.recover')}}</view>
							<navigator :url="`/pages/product/addGoods/index?mer_id=${merId}&product_id=${item.product_id}`" v-if="type != 5" class="shop_button" hover-class="none">{{$t(`message.tips.edit`)}}</navigator>
						</view>
					</view>
				</view>
			</block>
			...
			item.is_del == 1
				? Modal('温馨提示', `商品"${item.store_name}"将被删除，请问是否继续?`).then(res => {
						productDestory(this.merId, item.product_id)
							.then(res => {
								this.shopList.splice(index, 1);
								Toast(this.$t(`message.login.delSU`));
							})
							.catch(rej => {
								Toast(`${rej}`);
							});
