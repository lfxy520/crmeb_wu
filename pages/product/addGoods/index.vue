<template>
	<view class="container">
		<view class="step_one" v-if="step == 1">
			<view class="input_content">
				<view class="input_content_textarea">
					<textarea v-model="setFormData.store_name" :placeholder="$t('page.product.tip2')" placeholder-class="placeholderStyle" maxlength="60" />
					<view>
				...
				<view class="photo_count">
					<text class="photo_size">{{$t('page.product.tip4')}}</text>
					<view class="input_content_photo">
						...
						<view v-if="setFormData.imageList.length < 9" class="input_content_photo_adPh addBtn" @click="clk">
							...
							<view>{{$t('page.product.addpic')}}</view>
						</view>
					</view>
				</view>
				...
				<view class="input_content_describe">
					<view class="input_content_describe_title">
						<view class="input_content_describe_title_msg">{{$t('page.product.spdesc')}}</view>
					...
					<view class="input_content_describe_textarea">
						<textarea v-model="setFormData.store_info" value="" :placeholder="$t('page.product.tip3')" placeholderClass="placeholderClass" maxlength="200" />
					</view>
				</view>

				<view class="input_content_keyword">
					<view class="input_content_keyword_label">{{$t(`page.store.keyword`)}}</view>
					<view class="input_content_keyword_value"><input v-model="setFormData.keyword" type="text" value="" :placeholder="$t('page.product.tip5')" /></view>
				</view>
			</view>

			<view class="popup_group">
				<view class="popup_group_item" @click="selectPlatform">
					<view class="popup_group_item_label">{{$t('page.product.ptfenlei')}}</view>
					<view class="popup_group_item_value">
						<view class="popup_group_item_message">
							...
							<span v-else>{{$t('page.product.selectit')}}</span>
						</view>
				...
				<view class="popup_group_item" @click="selectStoreClass">
					<view class="popup_group_item_label">{{$t('page.product.dpfenlei')}}</view>
					<view class="popup_group_item_value">
						<view class="popup_group_item_message">
				...
				<view class="popup_group_item">
					<view class="popup_group_item_label">{{$t('page.product.danwei')}}</view>
					<view class="popup_group_item_value"><input v-model="setFormData.unit_name" type="text" :placeholder="$t('page.product.danweitip')" /></view>
				</view>
			</view>

			<view class="popup_group">
				<view class="radio">
					<view class="radio_label ">{{$t('page.product.ggxz')}}</view>
				...
				<view class="popup_group_item" @click="selectSpecification" v-if="setFormData.spec_type == 1">
					<view class="popup_group_item_label">{{$t('page.product.gg')}}</view>
					<view class="popup_group_item_value">
						<view class="popup_group_item_message">
			...
			<view class="popup_group">
				...
				<view class="popup_group_item" v-if="setFormData.delivery_way.includes('2')">
					<view class="popup_group_item_label">{{$t('page.product.baoyou')}}</view>
					<view class="popup_group_item_value">
						<view class="popup_group_item_message">
			...
			<view class="handle"><view class="handle_button" @click="handleNextStep">{{$t('page.product.next')}}</view></view>
		</view>
		...
		<avatar @upload="doUpload" @getName="getImgName" quality="1" ref="avatar" selWidth="250upx" selHeight="750upx"></avatar>
	</view>
</template>
...
export default {
	...
	data() {
		return {
			...
			productItem: {} || [],
			selectProductTitle: this.$t('page.product.ptfenlei'),
			specificationsRadioList: [
				{
					value: '0',
					name: this.$t('page.product.dangg')
				},
				{
					value: '1',
					name: this.$t('page.product.duogg')
				}
			],
			specifications: [
				{
					id: 5,
					label: this.$t('page.product.ggxz'),
					type: 'radio',
					model: 'spec_type',
					inforValue: '0',
					radioList: [
				...
				},
				{
					...
					value: '',
					DoNotShow: true,
					holder: this.$t('page.product.selectit'),
					jumpLogic: true // 是否为需要特数处理，执行跳转逻辑
				},
				{
					id: 6,
					label: this.$t('page.product.gg'),
					type: 'select',
					value: '',
					holder: this.$t('page.product.goset'),
					jumpLogic: true, // 是否为需要特数处理，执行跳转逻辑
					DoNotShow: true
				}
			],
	...
	onShow() {
		...
		if(getStorage('singleSpecification')) {
			this.setFormData.setSpecificaValue = this.$t('page.product.editprice');
		} else {
			this.setFormData.setSpecificaValue = '';
		}
		if(getStorage('attrValue')) {
			this.setFormData.setSpecificaValue2 = this.$t('page.product.editprice');
		} else {
			this.setFormData.setSpecificaValue2 = '';
		}
		if(this.step == 2) {
	...
	methods: {
				...
				let editGoodsDetils = res.data;
					...
					if(this.setFormData.spec_type == 1) {
						if(editGoodsDetils.attr.length) {
							this.setFormData.specifica = this.$t('page.product.editprice');
						}	
		...
		clk() {
			...
			avatar.fChooseImg(1,{selWidth: '750upx', selHeight: '750upx', inner: true});
		},
		doUpload(rsp) {
			let that = this
			console.log(rsp);
		...
		handleNextStep() {
			...
			let requireData = {
				image: this.$t('page.product.tip12'),
				store_name: this.$t('page.product.tip13'), // 商品名
				cate_id: this.$t('page.product.tip14'), // 平台分类
				mer_cate_id: this.$t('page.product.tip15'), // 店铺分类
				unit_name: this.$t('page.product.tip16'), // 商品单位
			};
			...
			if(this.setFormData.spec_type == '0' && !this.setFormData.setSpecificaValue){
					Toast(this.$t('page.product.tip17'));
					return;
