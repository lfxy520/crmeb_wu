	...
	<view class=" writeBg  ">
		<view class="container_input marginTop_none">
			<view class="container_input_item">
				<view class="container_input_item_label"><span>{{$t('page.product.commit')}}</span></view>
		...
		<view class="container_input marginTop_none">
			<view class="container_input_item">
				<view class="container_input_item_label"><span>{{$t('page.product.limit')}}</span></view>
				<view class="container_input_item_value">
					<input v-model="addGoodsSecoundData.once_count" type="number" value="" :placeholder="$t('page.product.tip7')" placeholder-class="inputPlaceHolder" />
			</view>
			</view>
			<view class="container_input_item">
				<view class="container_input_item_label"><span>{{$t('page.product.sort')}}</span></view>
				<view class="container_input_item_value">
					<input v-model="addGoodsSecoundData.sort" type="number" value="" :placeholder="$t('page.product.tip8')" placeholder-class="inputPlaceHolder" />
			</view>
			</view>
		</view>
		<view class="content_list">
			<view class="content_list_item" @click="inputGoodsDetils">
				<view>{{$t('page.goodsDetail.details')}}</view>
				<view class="content_list_item_han">
					<span v-if="disModel" style="color: #000000">{{$t('page.users.userAddressList.edit')}}</span>
					<span v-else>{{$t('page.product.setdetail')}}</span>
					<span class="iconfont">&#xe6bd;</span>
				</view>
			</view>
			<view class="content_list_video">
				<view class="content_list_video_title">{{$t('page.product.video')}}</view>
				<view class="video_list">
					...
					<view class="photo" @click="uploadVideo" v-else>
						...
						<view>{{$t('page.product.addvideo')}}</view>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="handle dobuButton">
			<view class="handle_button margin_right" @click="lastStep">{{$t('page.product.Previous')}}</view>
			<view class="handle_button" @click="submitCreatedGoods">{{$t('page.product.submit')}}</view>
		</view>
	</view>
</template>
<script>
			...
			if (this.product_id) {
				productUpdate(this.merId, this.product_id, postData)
					.then(res => {
						...
						hideLoading()
						Modal(this.$t('page.product.tip18'), '', {
