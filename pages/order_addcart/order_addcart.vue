	...
	<view :style="viewColor">
		<view class='shoppingCart'>
			<view class='labelNav acea-row row-around row-middle'>
				<view class='item'><text class='iconfont icon-xuanzhong'></text>{{$t(`page.goodsAddcart.labelNav`)}}</view>
			</view>
			<view class='nav acea-row row-between-wrapper'>
				<view>{{$t(`page.goodsAddcart.buyNum`)}} <text class='num t-color'>{{cartTotalCount}}</text></view>
				<view v-if="cartList.valid.length > 0 || cartList.invalid.length > 0" class='administrate acea-row row-center-wrapper'
				 @click='manage'>{{ footerswitch ? $t(`page.goodsAddcart.management`) : $t(`page.goodsAddcart.cancel`)}}</view>
			</view>
			<view v-if="cartList.valid.length > 0 || cartList.invalid.length > 0">
					...
					<block v-for="(item,index) in cartList.valid" :key="index">
						<view class='item acea-row row-between-wrapper'>
							<view class="store-title">
								...
								<view class="coupon-btn" v-if="item.hasCoupon>0" @click="giveCoupon(item)">{{$t('page.goodsDetail.coupons')}}</view>
							</view>
				...
				<view class='invalidGoods' v-if="cartList.invalid.length > 0">
					<view class='goodsNav acea-row row-between-wrapper'>
						<view @click='goodsOpen'><text class='iconfont' :class='goodsHidden==true?"icon-xiangxia":"icon-xiangshang"'></text>{{$t(`page.goodsAddcart.failureGoods`)}}</view>
						...
						<block v-for="(item,index) in cartList.invalid" :key='index'>
							<navigator :url="'/pages/goods_details/index?id='+item.product_id" class='item acea-row row-between-wrapper' hover-class='none'>
								...
								<view class='text acea-row row-column-between'>
									...
									<!-- <view class='infor line1' v-if="item.productInfo.attrInfo">{{$t(`page.goodsAddcart.attribute`)}}：{{item.productInfo.attrInfo.suk}}</view> -->
			...
			<view class='footer acea-row row-between-wrapper' v-if="cartList.valid.length > 0">
				...
				<view class='money acea-row row-middle' v-if="footerswitch==true">
					...
					<form @submit="subOrder" report-submit='true'>
						<button class='placeOrder' formType="submit">{{$t(`page.goodsAddcart.buyNow`)}}</button>
					</form>
				</view>
				<view class='button acea-row row-middle' v-else>
					<form @submit="subCollect" report-submit='true'>
						<button class='bnt bt-color' formType="submit">{{$t(`page.goodsAddcart.like`)}}</button>
					</form>
		...
		methods: {
			...
			getHostProduct: function() {
				...
				getProductHot(
					...
					that.loading = false;
					that.loadTitle = that.hotScroll ? this.$t(`page.goodsList.nono`) :  this.$t(`page.goodsList.more`);
				});
			},
			// 失效商品展开
			goodsOpen: function() {
