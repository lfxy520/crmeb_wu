	...
	<view class="pos-order-list" ref="container">
		<view class="nav acea-row row-around row-middle">
			...
			<view class="item" :class="where.status == 3 ? 'on' : ''" @click="changeStatus(3)">
				{{$t(`page.users.orderList.orderStatus[3].name`)}}
			</view>
			<view class="item" :class="where.status == 4 ? 'on' : ''" @click="changeStatus(4)">
				{{$t(`page.users.orderList.orderStatus[4].name`)}}
			</view>
			<view class="item" :class="where.status == 6 ? 'on' : ''" @click="changeStatus(6)">
				{{$t(`page.users.orderList.orderStatus[5].name`)}}
			</view>
		</view>
			<view class="list">
				<view class="item" v-for="(item, index) in list" :key="index">
					<view class="order-num acea-row row-middle">
						...
						{{$t(`page.orderDetails.orderId`)}}：{{ item.order_sn }}
						<span class="time">{{$t(`page.orderDetails.orderTime`)}}：{{ item.create_time }}</span>
					</view>
					...
					<view class="operation acea-row row-between-wrapper">
						...
						<view class="acea-row row-middle">
							<navigator class="bnt bnt_color" v-if="where.status == 2" :url="'/pages/admin/delivery/index?id='+item.order_id+'&merId='+merId">{{$t(`page.orderDetails.goship`)}}
							</navigator>
						</view>
					</view>
				</view>
		...
		<view v-if="!loading && list.length <= 0" class="nothing">
			...
			<view class="nothing_text">{{$t(`page.users.orderList.empty`)}}~</view>
		</view>
