<template>
	<view class="order-details pos-order-details">
		<view class="header acea-row row-middle">
			...
			<view class="data">
				<view class="order-num">{{$t(`page.orderDetails.orderId`)}}：{{ orderInfo.order_sn }}</view>
				<view>
		...
		<view class="remarks acea-row" v-if="goname != 'looks'" style="align-items: center;">
			...
			<view class="line1" style="text-align: left; flex: 1; margin-left: 10rpx;" @click="modify('1')">
				 {{orderInfo.remark ? orderInfo.remark : $t(`page.orderDetails.note`)}}
			</view>
		</view>
		...
		<view class="address">
			<view class="name">
				{{ orderInfo.real_name }}<span class="phone">{{ hidePhone(orderInfo.user_phone) }}</span>
			</view>
			<view>{{ orderInfo.user_address }}</view>
			<!-- #ifdef H5 -->
			<button class="copy copy-data" :data-clipboard-text="$t(`page.orderDetails.name`)+'：'+orderInfo.real_name+'\n'+$t(`page.orderDetails.phone`)+'：'+ orderInfo.user_phone+'\n'+$t(`page.orderDetails.address`)+'：'+orderInfo.user_address">{{$t(`page.orderDetails.copy`)}}</button>
			<!-- #endif -->
			<!-- #ifdef MP -->
			<!-- <button class="copy copy-data" @click="copyData('收货人姓名：'+orderInfo.real_name+'收货人电话：'+orderInfo.user_phone+'收货人地址：'+orderInfo.user_address)">{{$t(`page.orderDetails.copy`)}}</button> -->
			<button class="copy" @tap="copyText">{{$t(`page.orderDetails.copy`)}}</button>
			<!-- #endif -->
		</view>
		...
		<view class="public-total">
			{{ orderInfo.total_num }}{{$t('page.orderDetails.item')}}，{{$t(`page.orderDetails.payable`)}}
			<span class="money">{{ orderInfo.pay_price }}</span> ( {{$t(`page.orderDetails.freight`)}} ${{
		...
		<view class="wrapper">
			<view class="item acea-row row-between">
				...
				<view class="conter acea-row row-middle row-right">
					...
					<!-- #ifdef H5 -->
					<button class="copy copy-data" :data-clipboard-text="orderInfo.order_sn">{{$t(`page.orderDetails.copy`)}}</button>
					<!-- #endif -->
					<!-- #ifdef MP -->
					<span class="copy copy-data" @click="copyNum(orderInfo.order_sn)">{{$t(`page.orderDetails.copy`)}}</span>
					<!-- #endif -->
				</view>
			</view>
			<view class="item acea-row row-between">
			...
			<view class="item acea-row row-between">
				...
				<view class="conter">
					{{ orderInfo.paid == 1 ? $t(`page.orderDetails.payTrue`) : $t(`page.orderDetails.payFalse`) }}
				</view>
			</view>
			<view class="item acea-row row-between">
				<view>{{$t('page.orderDetails.payType')}}：</view>
				<view class="conter">{{ orderInfo.pay_type == 0 ? $t(`page.orderDetails.balance`) : (orderInfo.pay_type == 1 || orderInfo.pay_type == 2 || orderInfo.pay_type == 3) ? "微信支付" : "支付宝支付" }}</view>
			</view>
			<view class="item acea-row row-between">
		...
		<view class="wrapper">
			...
			<view class="actualPay acea-row row-right">
				{{$t(`page.orderDetails.actualPayment`)}}：<span class="money font-color-red">{{ orderInfo.pay_price }}</span>
			</view>
		</view>
		<view class="wrapper" v-if="orderInfo.delivery_type != '3' && orderInfo.order_type === 0 && orderInfo.delivery_type">
			...
			<view class="item acea-row row-between">
				...
				<view class="conter">
					...
					<span class="copy copy-data" :data-clipboard-text="orderInfo.delivery_id">{{$t(`page.orderDetails.copy`)}}</span>
				</view>
			</view>
		</view>
		...
		<view class="footer acea-row row-right row-middle" v-if="goname != 'looks'">
			...
			<view class="bnt cancel" @click="modify('0')" v-if="types == 0 && orderInfo.paid == 0">
				{{$t(`page.orderDetails.change`)}}
			</view>
			<view class="bnt cancel" @click="modify('0')" v-if="types == -1">
				{{$t(`page.orderDetails.refund`)}}
			</view>
			<view class="bnt cancel" @click="modify('1')">{{$t(`page.orderDetails.note`)}}</view>
			<view class="bnt cancel" v-if="orderInfo.pay_type === 'offline' && orderInfo.paid === 0" @click="offlinePay">
				{{$t(`page.orderDetails.nowPay`)}}
			</view>
			<navigator class="bnt delivery" v-if="types == 1" :url="'/pages/admin/delivery/index?id='+orderInfo.order_sn">{{$t(`page.orderDetails.goship`)}}</navigator>
		</view>
    ...
    hidePhone(phone) {
      ...
      return phone.substring(0, 3) + "****" + phone.substring(7);
			...
			copyText: function (e) {
			    ...
			    let arr = [
			      {
			        name: $t(`page.orderDetails.name`),
			        value: this.orderInfo.real_name
			      },
			      {
			        name: $t(`page.orderDetails.phone`),
			        value: this.orderInfo.user_phone
			      },
			      {
			        name: $t(`page.orderDetails.address`),
