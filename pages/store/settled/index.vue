	...
	<view v-if="!successful" :style="viewColor">
				...
				<view class='list'>
					<view class="item">
						<view class="acea-row row-middle">
							...
							<input type="text" maxlength="30" :placeholder="$t(`message.settled.emptyName`)" v-model="merchantData.enterprise_name" @input="validateBtn"
					...
					<view class="item">
						<view class="acea-row row-middle">
							...
							<input type="text" :placeholder="$t(`message.settled.emptyRealName`)" v-model="merchantData.user_name" @input="validateBtn" placeholder-class='placeholder' />
						</view>
					</view>
					<view class="item">
						<view class="acea-row row-middle">
							...
							<input type="text" :placeholder="$t(`message.settled.emptyPhone`)" v-model="merchantData.phone" @input="validateBtn" placeholder-class='placeholder' />
						</view>
					</view>
							...
							<view class="uni-list-cell">
								<view class="uni-list-cell-db acea-row row-middle">
									...
									<picker @change="bindPickerChange" :value="index" :range="array" range-key="category_name">
										<input :placeholder="$t(`message.settled.emptyCategory`)" type="text" readonly disabled v-model="mer_classification">
							...
							<view class="uni-list-cell">
								<view class="uni-list-cell-db acea-row row-middle">
									...
									<picker @change="bindPickerChange1" :value="index1" :range="storeTypeArr" range-key="type_name">
										<input :placeholder="$t(`message.settled.emptyStoretype`)" type="text" disabled readonly v-model="mer_storeType">
					...
					<view class="item no-border">
						<checkbox-group @change='ChangeIsAgree'>
							<checkbox class="checkbox" :checked="isAgree ? true : false" />{{$t(`page.store.agree`)}}</checkbox-group>
						<button class="settleAgree" @click="getConfig">《{{$t(`page.store.agreement`)}}》</button>
					</view>
					<button class='submitBtn' :class="validate === true ? 'on':''" @click="formSubmit">{{$t(`page.store.submit`)}}</button>
				</view>
			</view>
		</form>
		<view class="settlementAgreement" v-if="showProtocol">
			<view class="setAgCount">
				...
				<div class="title">{{isType ? '店铺类型说明' : this.$t(`page.store.agreement2`)}}</div>
				<view class="content">
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.mer_i_id) {
				...
				uni.showLoading({
					title: this.$t(`page.store.loading`),
				});
		...
		methods: {
			getConfig() {
				...
				// 获取配置
				this.protocol = this.sys_intention_agree.sys_intention_agree
			...
			async code() {
				...
				if (!that.merchantData.phone) return that.$util.Tips({
					title: this.$t(`message.settled.emptyPhone`)
				});
				await verify({
