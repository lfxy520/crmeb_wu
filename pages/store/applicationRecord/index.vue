	...
	<view class="">
		<view class="application-record" v-if="listData.length">
			<view class="card-list" v-for="item in listData" :key="item.mer_intention_id">
				<view class="card-top">
					...
					<view class="time">{{$t(`message.tips.time`)}}：{{item.create_time}}</view>
					<view v-if="item.fail_msg" class="reason">{{$t(`message.tips.reason`)}}：{{item.fail_msg}}</view>
				</view>
		...
		<view class='no-shop' v-if="!listData.length && !loading">
			<view class='pictrue' style="margin: 0 auto;">
				...
				<text>{{$t(`message.settled.appleTips`)}}</text>
			</view>
		</view>
	</view>
</template>
		...
		methods: {
			...
			statusText(number) {
				...
				let statusData = {
					...
					1: this.$t(`message.tips.adopt`),
					2: this.$t(`message.tips.noadopt`),
				};
