	...
	<view :style="{ 'background-image': `linear-gradient(0deg, rgba(0, 0, 0, 0.9) 0%, rgba(0, 0, 0, 0.5) 100%),url(${store.mer_banner})` }"
	 class="store-home">
		...
		<view class="header">
			...
			<navigator :url="'/pages/store/list/index?mer_id='+id" hover-class="none" class="search"><text class="iconfont icon-xiazai5"></text>{{$t('page.goodsDetail.search')}}</navigator>
		</view>
		<!-- #endif -->
		<!-- #ifdef H5 -->
		<view class="header">
		...
		<view v-show="navShow && tabActive === 1" class="nav" :style="viewColor">
			<view class="nav-cont">
				<view :class="{ active: navActive === 0 }" class="item" @click="navActive = 0;select.show = !select.show">
					<view class="cont">
						{{ select.selected ? $t('page.users.goodsCommentList.score') : $t('page.goodsList.default') }}
				...
				<view :class="{ active: navActive === 1 }" class="item" @click="set_where(2)">
					<view class="cont">{{$t('page.goodsDetail.sales')}}</view>
				</view>
		...
		<scroll-view class="main" scroll-y="true" @scroll="scrollHome" :style="viewColor">
			...
			<view id="store" class="store">
				...
				<view class="text">
					<navigator :url="`/pages/store/detail/index?id=${id}`" hover-class="none">
						...
						<text v-else-if="store.is_trader" class="font-bg-red">{{$t(`page.store.selfSupport`)}}</text>
			...
			<view v-show="!navShow && tabActive === 1" class="nav">
				<view class="nav-cont">
					<view :class="{ active: navActive === 0 }" class="item" @click="navActive = 0;select.show = !select.show">
						<view class="cont">
							{{ select.selected ? $t(`page.goodsSearch.score`) : $t(`page.goodsSearch.Default`) }}
							<text :class="['arrow-icon', 'iconfont', select.show ? 'icon-xiangshang' : 'icon-xiangxia']"></text>
						</view>
					</view>
					<view :class="{ active: navActive === 1 }" class="item" @click="set_where(2)">
			...
			<view class="tab-cont" :style="viewColor">
							...
							<view v-for="item in goods" :key="item.product_id" class="item" @click="goGoodsDetail(item)">
								...
								<view class="text on acea-row row-between-wrapper">
									...
									<view class="item_bot">
										...
										<view class="score">{{ item.rate }}{{$t('page.users.goodsCommentList.good')}} {{ item.reply_count }}{{$t('page.users.goodsCommentList.score')}}</view>
									</view>
					...
					<view class="category">
						<view class="section">
							<view class="head" @click="goCategoryGoods('')">
								<view class="title">{{$t('page.users.goodsCommentList.all')}}</view>
						...
						<view v-for="item in productList" :key="item.product_id" class="item" @click="goGoodsDetail(item)">
							...
							<view class="text acea-row row-between-wrapper">
								...
								<view class="item_bot">
									...
									<view class="score">{{ item.rate }}评分 {{ item.reply_count }}{{$t('page.users.goodsCommentList.score')}}</view>
								</view>
							</view>
							<!--<view v-if="item.max_extension && (item.product_type == 0 || item.product_type == 2)" class="foot">
								<text v-show="!isColumn" class="iconfont"></text>
				...
				<view v-show="tabActive === 3">
					...
					<emptyPage v-if="coupon.length == 0" :title="$t(`page.users.orderConfirm.nonecoupons`)"></emptyPage>
				</view>
				<!--活动专题-->
	...
	export default {
		...
		data() {
			return {
				...
				select: {
					...
					options: [{
							id: 0,
							name: this.$t(`page.goodsSearch.Default`)
						},
						{
							id: 1,
							name: this.$t(`page.goodsSearch.score`),
						}
					]
				},
				// 底部菜单
		...
		computed: {
			score: function() {
				...
				// 如果有评分则使用评分，没有则默认100分
				if ('service_score' in store && store.service_score) {
					score.number = parseFloat(store.service_score);
				} else {
					score.number = 100; // 默认100分
				}
				// 转换为百分比显示
		...
		methods: {
			...
			get_service_list: function(isPage) {
				...
				storeServiceList(that.id, that.where).then(res => {
					...
					that.loading = false;
					that.loadTitle = loadend ? this.$t(`page.goodsList.more`) : this.$t(`page.goodsList.no`);
			...
			getCategory: function() {
				...
				getStoreCategory(this.id).then(res => {
					...
					this.loadTitle = this.$t(`page.goodsList.no`);
				});
			},
			// 获取商铺优惠券
			getCoupon: function() {
