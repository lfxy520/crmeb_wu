			...
			<view v-if="invoiceList && invoiceList.length" class="list">
				<view v-for="(item,index) in invoiceList" :key="index" class="item">
					...
					<view class="acea-row row-right item-ft">
						<view class="btn" @tap="editInvoice(item.user_receipt_id)"><text class="iconfont icon-bianji"></text>{{$t(`message.tips.edit`)}}</view>
						<view class="btn" @tap="deleteInvoice(item.user_receipt_id,index)"><text class="iconfont icon-shanchu"></text>{{$t(`page.users.userAddressList.del`)}}</view>
					</view>
				</view>
			</view>
			<view v-else class="nothing">
