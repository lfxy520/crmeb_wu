			...
			<view class='wrapper'>
				<view class='header'>
					<view class='headerCon'>
						<view class='account acea-row row-top row-between'>
							<view class='assets'>
								<view>{{$t(`page.users.userBill.Total`)}}</view>
								<view class='money'>{{userInfo.now_money || 0}}</view>
							</view>
							<!-- #ifdef APP-PLUS || H5 -->
							<navigator v-if="recharge_switch == 0" url="/pages/users/user_payment/index" hover-class="none" class='recharge t-color'>{{$t(`page.users.userBill.Recharge`)}}</navigator>
							<!-- #endif -->
							<!-- #ifdef MP -->
							<view v-if="recharge_switch == 0" @click="openSubscribe('/pages/users/user_payment/index')" class='recharge t-color'>{{$t(`page.users.userBill.Recharge`)}}</view>
							<!-- #endif -->
						</view>
						<view class='cumulative acea-row row-top'>
							<!-- v-if="balance_func_status == 1" -->
							<view class='item' >
								<view>{{$t(`page.users.userBill.Recharge`)}}</view>
								<view class='money'>{{userInfo.total_recharge || 0}}</view>
							</view>


							<view class='item'>
								<view>{{$t(`page.users.userBill.Consumption`)}}</view>
				...
				<view class='nav acea-row row-middle'>
					<navigator class='item' hover-class='none' url='/pages/users/user_bill/index'>
						...
						<view>{{$t(`page.users.userBill.All`)}}</view>
					</navigator>
