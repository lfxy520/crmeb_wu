	...
	<view :style="viewColor">
		<view class='address-management' :class='addressList.length < 1 && page > 1 ? "fff":""'>
			...
			<radio-group class="radio-group" @change="radioChange" v-if="addressList.length">
				<view class='item' v-for="(item,index) in addressList" :key="index">
					<view class='address' @click='goOrder(item.address_id)'>
						...
						<view>{{$t('page.orderDetails.address')}}：{{item.province || ''}}{{item.city || ''}}{{item.district || ''}}{{item.street || ''}}{{item.detail || ''}}</view>
					</view>
					<view class='operation acea-row row-between-wrapper'>
						...
						<view class='acea-row row-middle'>
							<view @click='editAddress(item.address_id)'><text class='iconfont icon-bianji'></text>{{$t(`page.users.userAddressList.edit`)}}</view>
							<view @click='delAddress(index)'><text class='iconfont icon-shanchu'></text>{{$t(`page.users.userAddressList.del`)}}</view>
						</view>
					</view>
				</view>
			</radio-group>
			...
			<view class='footer acea-row row-between-wrapper'>
				<view class='addressBnt b-color on' @click='addAddress'><text class='iconfont icon-tianjiadizhi'></text>{{$t(`page.users.userAddressList.add`)}}</view>
			</view>
		</view>
