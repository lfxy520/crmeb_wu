				...
				<view class='item' v-for="(item,index) in orderList" :key="index">
					<view>
						...
						<view class='bottom acea-row row-right row-middle'>
							<view v-if="item.presellOrder.activeStatus === 2" class='bnt cancelBnt' @click.stop='cancelOrder(index,item.order_id)'>{{$t(`page.orderDetails.cancelOrder`)}}</view>
							<view v-if="item.presellOrder.activeStatus === 1" class='bnt b-color' @click.stop='goPay(item.presellOrder.pay_price,item.order_id)'>{{$t(`page.orderDetails.nowPay`)}}</view>
	...
	export default {
		...
		data() {
			return {
				...
				payMode: [
					{
						name: this.$t(`page.orderDetails.total`),
						icon: "icon-yuezhifu",
						value: 'balance',
						title: '',
						number: 0,
