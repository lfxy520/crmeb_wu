	...
	<view :style="viewColor">
		<view class='cash-withdrawal' :hidden='!loading'>
			<view class='nav acea-row  row-between-wrapper'>
				<view class='name'>{{$t(`page.users.userSpread.txz`)}}</view>
			...
			<view class='wrapper'>
				<view :hidden='currentTab != 0' class='list'>
					<form @submit="subCash" report-submit='true'>
						<view class='item acea-row row-between-wrapper'>
							...
							<view class='input'><input placeholder='' placeholder-class='placeholder' name="real_name"></input></view>
						</view>
						<view class='item acea-row row-between-wrapper'>
						...
						<view class='item acea-row row-between-wrapper'>
							...
							<view class='input'><input type='number' placeholder='' placeholder-class='placeholder' name="bank_address"></input></view>
						</view>
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`page.users.userSpread.amount`)}}:</view>
							<view class='input'><input placeholder='' placeholder-class='placeholder' name="extract_price"
								 type='digit' v-model="extract_price"></input></view>
						</view>
						<view class='tip mt25'>
							{{$t(`page.users.userSpread.tips`)}}: <text class="price">{{userInfo.brokerage_price}}</text>
						</view>
						<view class='tip'>
				...
				<view :hidden='currentTab != 3' class='list'>
					<form @submit="subCash" report-submit='true'>
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`page.users.userSpread.trc20`)}}:</view>
							<view class='input'><input placeholder='' placeholder-class='placeholder' name="usdt_code"></input></view>
						</view>
						<view class='item acea-row row-between-wrapper'>
							<view class='name'>{{$t(`page.users.userSpread.amount`)}}:</view>
							<view class='input'><input placeholder='' placeholder-class='placeholder' name="extract_price"
	...
	export default {
		...
		methods: {
			...
			subCash: function(e) {
				...
				} else if (that.currentTab == 3) { //usdt
					value.extract_type = 'usdt';
					if (value.usdt_code.length == 0) return this.$util.Tips({
						title: this.$t(`page.users.userSpread.tips5`)
					});
				}
				if (value.extract_price.length == 0) return this.$util.Tips({
					title: this.$t(`page.users.userSpread.tips3`)
				});
				if (Number(value.extract_price) < that.minPrice) return this.$util.Tips({
					title: this.$t(`page.users.userSpread.tips4`) + that.minPrice
				});
				value.extract_type = this.currentTab
				if(this.currentTab == 3)
				value.extract_type = 5;
...
<style lang="scss">
	...
	.icon-icon88{
		padding-left: 10px;display: inline-block;text-align: center;line-height: 28px;margin-right: 10px;border-radius: 50%;
	}
    .mt25{
		margin-top: 25rpx;
	}
