		...
		<view class='coupon-list' v-if="couponsList.length">
			<view v-for='(item,index) in couponsList' :key="index">
				<view v-if="item.merchant && item.status==0" class="store-info acea-row row-between-wrapper">
					...
					<view v-if="hide_mer_status == 0 && item.status == 0" class="store" @click="goStore(item)"></view>
				</view>
				<view class='item acea-row row-center-wrapper'>
					<view v-if="item.status == 0" class='money' :style="{ 'background-image': `url(${domain}/static/diy/couponBg${keyColor}.png)` }">
						...
						<view class="pic-num">{{$t(`page.users.userCoupon.min`)}}{{ item.use_min_price }}</view>
					</view>
					...
					<view class='text'>
						...
						<view class='data acea-row row-between-wrapper'>
							...
							<view v-if="item.status==0">
								...
								{{$t(`page.users.userCoupon.can`)}}
								</navigator>
							</view>
							<view class='bnt1 gray' style="position: static;" v-if="item.status==1">{{$t(`page.users.userCoupon.can`)}}</view>
							<view class='bnt1 gray' style="position: static;" v-if="item.status==2">{{$t(`page.users.userCoupon.used`)}}</view>
						</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				tabList:[
					{
						title:this.$t(`page.users.userCoupon.available`)
					},
					{
						title:this.$t(`page.users.userCoupon.expired`)
					}
				],
				limit:15,
				page:1,
