		...
		<view class='order-submission'>
					...
					<view class='addressCon' v-if="addressInfo.real_name">
						...
						<view>
							<text class='default t-color'
								v-if="addressInfo.is_default"></text>{{addressInfo.province}}{{addressInfo.city}}{{addressInfo.district}}{{addressInfo.street || ''}}{{addressInfo.detail}}
						</view>
					</view>
				...
				<view class="store-item" v-for="(item,index) in cartInfo" :key="index">
					...
					<view class="wrapper">
						<view class='item acea-row row-between-wrapper'
							...
							<block v-else>
								<view class='discount'>{{$t(`page.users.orderConfirm.nonecoupons`)}}</view>
							</block>
						</view>
						...
						<view class='item acea-row row-between-wrapper' v-if="textareaStatus">
							<view style="width: 100px;">{{$t(`page.users.orderConfirm.note`)}}</view>
							<input v-if="coupon.status===false" placeholder-class='placeholder'
								@input='bindHideKeyboard' value="" name="mark" :placeholder='$t(`page.users.orderConfirm.placeNote`)'
								v-model="msgObj[item.mer_id]"></input>
						</view>
						<view class="total">
							{{item.order.total_num}}{{$t(`page.users.orderConfirm.items`)}} {{$t(`page.users.orderConfirm.total`)}}
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<view class='settlement' :class='couponData.status != "noAddress" ? "" : "disabled"' style='z-index:100'
					@tap="SubOrder">{{couponData.status != "noAddress" ? $t(`page.users.orderConfirm.payPal`):$t(`message.tips.selectAddress`)}}</view>
			</view>
		</view>
		<block v-if="coupon.status">
			<couponListWindow :coupon='coupon' :couponTitle="plantCoupon ? $t(`page.users.orderConfirm.coupons`) : $t(`page.users.orderConfirm.coupons`)" @ChangCouponsClone="ChangCouponsClone" @getCoupon="getCoupon"
		...
		data() {
			return {
				...
				cartArr: [
					...
					{
						"name": this.$t(`page.orderDetails.offline`),
						"icon": "icon-yinhangqia",
						value: 'offline',
						title: this.$t(`page.orderDetails.offline`),
						payStatus: 2,
					},
				],
				tagStyle: {
		...
		methods: {
				...
				createOrder(data).then(res => {
					...
					switch (status) {
						...
						case "weixinApp":
							...
							this.$wechat.pay(jsConfig).then(res => {
								...
								if (res.errMsg == 'chooseWXPay:cancel') return that.$util.Tips({
									title: this.$t(`page.orderPayStatus.cancel`)
								}, {
									tab: 5,
							...
							uni.requestPayment({
								...
								success: (e) => {
									let url = '/pages/order_pay_status/index?order_id=' + orderId +
										'&msg=支付成功';
									return that.$util.Tips({
										title: this.$t(`page.orderPayStatus.success`),
										icon: 'success'
									}, {
										tab: 4,
										url: url
								...
								fail: (e) => {
									uni.showModal({
										content: this.$t(`page.orderPayStatus.fail`),
										showCancel: false,
										success: function(res) {
											let url =
						...
						case "balance":
							...
							// #ifdef H5
						case 'online':
							let location = `${jsConfig}`;
							if(!location) 
							{
								let host = window.location.protocol + "//" + window.location.host;
								location = `${host}/pages/order_pay_status/index?order_id=${orderId}`;
							}
							setTimeout(() => {
			...
			SubOrder: function(e) {
					...
					for(var i=0; i<that.order_extend.length; i++){
						...
						if (el.require) {
							if(!el.value || el.value.length == 0){
								return that.$util.Tips({
									title: this.$t(`users.orderConfirm.fillright`)+el.title
								});
				...
				if (data.payType == 'balance' && parseFloat(that.userInfo.now_money) < parseFloat(that.totalPrice))
					return that.$util
						.Tips({
							title: this.$t(`message.orderConfirm.notenough`)
						});
				uni.showLoading({
