		...
		<form @submit="formSubmit" report-submit='true'>
			<view class='personal-data'>
						 ...
						 :key="index">
							<view class="picTxt acea-row row-between-wrapper">
								...
								<view class="pictrue" v-else>
									<image  :src='item.avatar' v-if="item.avatar"></image>
									<image src="/static/images/f.png" v-else></image>
									<view class="radius" v-else><span class="iconfont iconfonta icon-jilu-xuanzhong"></span></view>
								</view>
								<view class="text">
				...
				<view class='list' style="margin-top: 1rpx;">
					...
					<view class='item acea-row row-between-wrapper'>
						<view>{{$t(`page.orderDetails.phone`)}}</view>
					...
					<view class='item acea-row row-between-wrapper'>
						...
						<view class='input acea-row row-between-wrapper'>
							<input type='text' :placeholder="$t(`message.tips.edidM`)" disabled='true' class='id'></input>
							<text class='iconfont icon-you'></text>
						</view>
					</view>
					<!--<view class='item acea-row row-between-wrapper'>
				...
				<button class='modifyBnt bg-color' formType="submit">{{$t('page.users.userInfo.save')}}</button>
				<!-- #ifdef H5 -->
				<view class="logOut acea-row row-center-wrapper" @click="outLogin" v-if="!this.$wechat.isWeixin()">{{$t('page.users.userInfo.logOut')}}</view>
				<!-- #endif -->
				<!-- #ifdef MP || APP-PLUS -->
				<view class="logOut acea-row row-center-wrapper" @click="outLogin">{{$t('page.users.userInfo.logOut')}}</view>
				<!-- #endif -->	
			</view>
		</form>
