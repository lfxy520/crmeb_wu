	...
	<view>
		<view class='return-list' v-if="orderList.length">
			<view class='goodWrapper' v-for="(item,index) in orderList" :key="index" @click='goOrderDetails(item.order_id)'>
				...
				<view class='totalSum'>{{item.cartInfo.length || 0}}{{$t(`page.orderDetails.item`)}}，{{$t(`page.orderDetails.total`)}} <text class='font-color price'>{{item.pay_price}}</text></view>
			</view>
		</view>
		...
		methods: {
			...
			getOrderList: function() {
				...
				getOrderList({
					...
					that.loading = false;
					that.loadTitle = loadend ? this.$t(`page.goodsList.more`) :  this.$t(`page.goodsList.more`);
