								...
								<div class="acea-row">
												...
												<div class="broadcast-details_box noPad" v-if="inx == 0">
													<div class="broadcast_details_img">
														...
														<div class="broadcast_details_model">
															{{item.refundOrder.refund_num}}{{$t(`page.orderDetails.item`)}}
														</div>
													</div>
									...
									<div v-if="item.msn_type === 5 &&  item.orderInfo && item.orderInfo.order_id">
										<div class="broadcast-details_num acea-row row-middle">
											<span>{{$t(`page.orderDetails.orderId`)}}：{{ item.orderInfo.order_sn }}</span>
											<!-- #ifdef H5 -->
											<button class="copy copy-data" :data-clipboard-text="item.orderInfo.order_sn">{{$t(`page.orderDetails.copy`)}}</button>
											<!-- #endif -->
											<!-- #ifndef H5 -->
											<button class="copy" @tap.stop="copyText(item.orderInfo.order_sn)">{{$t(`page.orderDetails.copy`)}}</button>
											<!-- #endif -->
										</div>
											...
											<div class="broadcast-details_order noPad" v-for="(val, inx) in item.orderInfo.orderProduct" :key="val.id">
												<div class="broadcast-details_box noPad" v-if="inx == 0">
													<div class="broadcast_details_img">
														...
														<div class="broadcast_details_model">
															{{item.orderInfo.total_num}}{{$t(`page.orderDetails.item`)}}
														</div>
													</div>
													<div class="broadcast_details_picBox noPad">
							...
							<div class="text textR">
								...
								<div class="acea-row ">
									...
									<navigator v-if="item.msn_type === 6 && item.refundOrder.refund_order_id" open-type="redirect">
										<div class="broadcast-details_num acea-row row-middle">
											<text>{{$t(`page.orderDetails.orderId`)}}：{{ item.refundOrder.refund_order_sn }}</text>		
											<!-- #ifdef H5 -->
											<button class="copy copy-data" :data-clipboard-text="item.refundOrder.refund_order_sn">{{$t(`page.orderDetails.copy`)}}</button>
											<!-- #endif -->
											<!-- #ifndef H5 -->
											<button class="copy" @tap.stop="copyText(item.refundOrder.refund_order_sn)">{{$t(`page.orderDetails.copy`)}}</button>
											<!-- #endif -->
										</div>
										<div class="conter acea-row row-middle">
									...
									<div v-if="item.msn_type === 5 && item.orderInfo.order_id">
										<div class="broadcast-details_num acea-row row-middle">
											<text>{{$t(`page.orderDetails.orderId`)}}：{{ item.orderInfo.order_sn }}</text>
											<!-- #ifdef H5 -->
											<button class="copy copy-data" :data-clipboard-text="item.orderInfo.order_sn">{{$t(`page.orderDetails.copy`)}}</button>
											<!-- #endif -->
											<!-- #ifndef H5 -->
											...
											<div class="broadcast-details_order noPad" v-for="(val, inx) in item.refundOrder.refundProduct">
												<div class="broadcast-details_box noPad" v-if="inx == 0">
													...
													<div class="broadcast_details_picBox noPad">
														...
														<div class="broadcast_details_pic">
															{{item.refundOrder.refund_num}}{{$t(`page.orderDetails.item`)}}，
															{{$t(`page.orderDetails.total`)}} {{ item.refundOrder.refund_price }}
														</div>
													</div>
												</div>
											</div>
					...
					<div class="broadcast-details_order footer_count">
						...
						<div class="broadcast-details_box" v-if="productId && productInfo.product_id">
							...
							<div class="broadcast_details_picBox">
								...
								<div class="acea-row row-between product_price">
									...
									<div class="broadcast_details_btn product_btn" @click="sendProduct">{{$t(`message.login.send`)}}</div>
								</div>
							</div>
						</div>
						<!-- 订单信息 -->
						<div class="broadcast_box" v-if="orderId && orderInfo.order_id">
							...
							<div class="broadcast-details_num broadcast_num acea-row row-between-wrapper">
								<text>{{$t(`page.orderDetails.orderId`)}}：{{ orderInfo.order_sn }}</text>
							...
							<div class="broadcast-details_box">
								...
								<div class="broadcast_details_picBox">
									...
									<div>
										<div class="broadcast_details_pic">
											{{ orderInfo.total_num }}{{$t(`page.orderDetails.item`)}}，{{$t(`page.orderDetails.total`)}} <text class="p-color">{{ orderInfo.pay_price }}</text>
										</div>
						...
						<div class="broadcast_box" v-if="refund_order_id && refundDetail.refund_order_id">
							...
							<div class="broadcast-details_box">
								...
								<div class="broadcast_details_picBox">
									...
									<div>
										<div class="broadcast_details_pic">
											{{ refundDetail.refund_num }}{{$t(`page.orderDetails.item`)}}，
											{{$t(`page.orderDetails.total`)}} <text class="p-color">{{ refundDetail.refund_price}}</text>
										</div>
			...
			<form>
				<div class="footer acea-row row-between row-bottom" ref="footer">
					...
					<view class="input_count">
						<input type="text" placeholder-class='placeholder' :placeholder="$t(`page.chat.text`)" class="input" ref="input" v-show="!voice" @input="bindInput" @keyup="keyup"
	...
	export default {
		...
		mounted: function() {
			...
			uni.$on("socket_error", () => {
				if (!that.isBack) {
					let self = this
					uni.showModal({
						title: this.$t(`page.chat.prompt`),
						content: this.$t(`page.chat.tips`),
						success: function(res) {
							if (res.confirm) {
								self.wsStart()
							} else if (res.cancel) {
		...
		methods: {
			...
			start() {
				...
				that.speak = this.$t(`page.chat.speak`);
				that.recording = true;
			},
			move() {
				...
				this.speak = this.$t(`page.chat.speak`);
