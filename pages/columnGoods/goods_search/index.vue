		...
		<view class='searchGood'>
			<view class='search acea-row row-between-wrapper'>
				<view class='input acea-row row-between-wrapper'>
					...
					<input type='text' :value='searchValue' :focus="focus" :placeholder='$t(`page.goodsSearch.place`)' placeholder-class='placeholder'
					 @input="setValue"></input>
				</view>
				<view class='bnt' @tap='searchBut'>{{$t(`page.goodsSearch.search`)}}</view>
			</view>
			<view class='title'> <text class="iconfont icon-shanchu" @click="remove"></text></view>
			...
			<view>
				<view class="more-btn" v-if="historyList.length>9 && !historyBox" @click="historyBox = true">
					{{$t(`page.goodsSearch.open`)}}<text class="iconfont icon-xiangxia"></text>
				</view>
				<view class="more-btn" v-if="historyList.length>9 && historyBox" @click="historyBox = false">
					{{$t(`page.goodsSearch.up`)}}<text class="iconfont icon-xiangshang"></text>
				</view>
			</view>
			<view v-if="historyList.length == 0" style="text-align: center; color: #999;"></view>
