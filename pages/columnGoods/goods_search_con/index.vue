		...
		<view class='productList'>
			<view class='search acea-row row-between-wrapper'>
				...
				<view class='input acea-row row-between-wrapper'><text class='iconfont icon-sousuo'></text>
					<input :placeholder='$t(`page.goodsSearch.place`)' placeholder-class='placeholder' confirm-type='search' name="search" :value='where.keyword'
			...
			<view class="nav-wrapper">
				<view v-if="hide_mer_status == 0" class="tab-bar">
					<view class="tab-item" :class="{on:tabIndex==1}" @click="tabIndex = 1">{{$t(`page.goodsSearch.goods`)}}</view>
					<view class="tab-item" :class="{on:tabIndex==2}" @click="tabIndex = 2">{{$t(`page.goodsSearch.mer`)}}</view>
				</view>
						...
						<view class='text on acea-row row-between-wrapper'>
							...
							<view class="item_bot">
								...
								<view class="item_tags acea-row">
									...
									<text v-else-if="item.merchant.is_trader && item.product_type == 0" class="font-bg-red bt-color line1">{{$t(`page.store.selfSupport`)}}</text>
							...
							<view class="left-wrapper">
								...
								<view class="con-box">
									<view class="name acea-row row-between-wrapper line1">
										...
										<text v-else-if="item.is_trader" class="font-bg-red bt-color">{{$t(`page.store.selfSupport`)}}</text>
									</view>
									<view class="star-box">
										<view class="star">
	...
	export default {
		...
		data() {
			return {
				...
				shopTabs: [{
						title: this.$t(`page.goodsSearch.Default`),
						key: 0,
						order: ""
					},
					{
						title: this.$t(`page.goodsSearch.Seller`),
						key: 1,
						order: 'sales'
					},
					{
						title: this.$t(`page.goodsSearch.score`),
						key: 2,
						order: 'rate'
					},
					{
						title: this.$t(`page.goodsSearch.location`),
						key: 3,
						order: 'location'
					}
				],
				...
				downMenu: [{
					...
					{
						title: this.$t(`page.goodsSearch.new`),
						key: 3,
					}
				],
				// 是否第一个
		...
		onReachBottom() {
			if (this.tabIndex == 1) {
				...
				if (this.count === this.storeList.length) {
					...
					uni.showToast({
						title: this.$t(`page.goodsList.no`),
						icon: 'none',
						duration: 1000
					});
				} else {
