	...
	<view :style="viewColor">
		<view class="product-window" :class="(attr.cartAttr === true ? 'on' : '') + ' ' + (iSbnt || destri?'join':'') + ' ' + (isPresell ?'presell-window':'product-window')">
			<view class="textpic acea-row row-between-wrapper">
				<view class="pictrue">
					<image :src="attr.productSelect && attr.productSelect.image ? attr.productSelect.image : image" @click="loookImg"></image>
				</view>
				<view class="text">
					<view class="line1">
			...
			<view class="joinBnt b-color" v-if="destri && attr.productSelect.stock>0" @click="goCat">{{$t(`page.goodsDetail.buyNow`)}}</view>
			<view class="joinBnt on" v-else-if="destri && attr.productSelect.stock<=0">{{$t(`page.goodsDetail.soldOut`)}}</view>
			<view class="joinBnt b-color" v-if="iSbnt && attr.productSelect.stock>0"
			 @click="goCat">我要参团</view>
			<view class="joinBnt on" v-else-if="(iSbnt && attr.productSelect.stock<=0)">{{$t(`page.goodsDetail.soldOut`)}}</view>
		</view>
