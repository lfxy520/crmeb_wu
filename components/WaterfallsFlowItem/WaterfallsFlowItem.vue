    ...
    <view v-if="type == 0" class="wf-item-page wf-page0" :style="viewColor">
		...
		<view class="text">
			...
			<view class="item_tags">
				<text v-if="item.product_type == 0 && item.merchant && item.merchant.type_name" class="font-bg-red b-color">{{item.merchant.type_name}}</text>
				<text v-else-if="item.product_type == 0 && item.merchant && item.merchant.is_trader" class="font-bg-red b-color">{{$t(`page.store.selfSupport`)}}</text>
	...
	<view v-else-if="type == 1" class="wf-page1" :style="viewColor">
		...
		<view class='text'>
			...
			<view class="item_tags acea-row">
				<text v-if="item.merchant && item.merchant.type_name && item.product_type == 0" class="font-bg-red b-color">{{item.merchant.type_name}}</text>
				<text v-else-if="item.merchant && item.merchant.is_trader && item.product_type == 0" class="font-bg-red b-color">{{$t(`page.store.selfSupport`)}}</text>
				<text v-if="item.product_type != 0" :class="'font_bg-red type'+item.product_type">{{item.product_type == 1 ? "秒杀" : item.product_type == 2 ? "预售" : item.product_type == 3 ? "助力" : item.product_type == 4 ? "拼团" : ""}}</text>
				<text class="tags_item ticket" v-if="item.issetCoupon">{{$t(`page.store.collar`)}}</text>
				<text class="tags_item delivery" v-if="item.delivery_free == 1">{{$t(`page.goodsDetail.shipping`)}}</text>
			</view>
