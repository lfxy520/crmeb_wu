<template>
	<view>
		<view class='coupon-list-window animated' :class='coupon.status==true?"slideInUp":""'>
			<view class='title'>
				<view class="item">{{$t('page.goodsDetail.coupons')}}</view>
			</view>
			<block v-if="couponArr.length">
				...
				<view class="foot-box">
					<view class="left">
						{{$t('page.goodsDetail.choose')}}{{allNum}}张，可优惠<text>{{allCouponNum}}</text>
					</view>
					<view class="btn" @click="confirm">{{$t('page.goodsDetail.confirm')}}</view>
				</view>
			</block>
			<!-- 无优惠券 -->
